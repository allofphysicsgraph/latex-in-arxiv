...
class MyService(rpyc.Service):
    ...
    def exposed_process_data_set(self, path=False, file_names=[]):
        ...
        for f_name in tqdm(file_names):
            ...
            x.exposed_get_abstract(data=file_data)
            x.exposed_get_equation(data=file_data)
            x.exposed_get_figure(data=file_data)

            y.exposed_get_caption(data=file_data)
            y.exposed_get_bibitem(data=file_data)
            y.exposed_get_usepackage(data=file_data)
            y.exposed_get_affiliation(data=file_data)
            y.exposed_get_author(data=file_data)
            y.exposed_get_title(data=file_data)
            y.exposed_get_section(data=file_data)
            y.exposed_get_label(data=file_data)
            y.exposed_get_cite(data=file_data)
            y.exposed_get_ref(data=file_data)
