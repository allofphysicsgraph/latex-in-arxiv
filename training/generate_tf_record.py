...
from transformers import BertTokenizerFast  # For a basic BERT tokenizer (replace with yours)
import random

# 1. Define a Very Simple Vocabulary (Illustrative - Replace with your vocab.txt)
vocab_list = ["[PAD]", "[UNK]", "[CLS]", "[SEP]", "[MASK]", "documentclass", "article", "usepackage",
             "graphicx", "begin", "document", "section", "introduction", "text", "equation",
             "frac", "1", "2", "alpha", "beta", "end", "abstract"]
...
for index, token_id in enumerate(input_ids):
    ...
    if random.random() < masking_probability:
        ...
        # 10% of the time, replace with a random token (from vocab - excluding special tokens for simplicity here)
            ...
            random_token_id = random.choice([id for id in range(len(vocab_list)) if id not in tokenizer.all_special_ids])
...
input_ids_padded = output_input_ids[:max_seq_length] + [tokenizer.pad_token_id] * (max_seq_length - min(len(output_input_ids), max_seq_length))
attention_mask = [1] * min(len(output_input_ids), max_seq_length) + [0] * (max_seq_length - min(len(output_input_ids), max_seq_length))
token_type_ids = [0] * max_seq_length # All 0s for single document pre-training

# Masked LM positions and labels also need padding
masked_lm_positions_padded = masked_lm_positions[:max_predictions_per_seq] + [0] * (max_predictions_per_seq - min(len(masked_lm_positions), max_predictions_per_seq)) # Pad with 0 - positions are indices, so 0 is usually safe padding
masked_lm_ids_padded = masked_lm_labels[:max_predictions_per_seq] + [0] * (max_predictions_per_seq - min(len(masked_lm_labels), max_predictions_per_seq)) # Pad with 0 - or pad_token_id if you prefer
masked_lm_weights = [1.0] * min(len(masked_lm_positions), max_predictions_per_seq) + [0.0] * (max_predictions_per_seq - min(len(masked_lm_positions), max_predictions_per_seq)) # Weights for loss


# 7. Create tf.train.Example
example = tf.train.Example(features=tf.train.Features(feature={
    "input_ids": tf.train.Feature(int64_list=tf.train.Int64List(value=input_ids_padded)),
    ...
    "masked_lm_positions": tf.train.Feature(int64_list=tf.train.Int64List(value=masked_lm_positions_padded)),
    "masked_lm_ids": tf.train.Feature(int64_list=tf.train.Int64List(value=masked_lm_ids_padded)),
    "masked_lm_weights": tf.train.Feature(float_list=tf.train.FloatList(value=masked_lm_weights)),
}))
