// SOURCE: #https://github.com/yaml/libyaml/blob/master/tests/run-dumper.c
#include <yaml.h>
...
int copy_document(yaml_document_t *document_to,
...
int compare_nodes(yaml_document_t *document1, int index1,
  ...
  case YAML_SEQUENCE_NODE:
    if ((node1->data.sequence.items.top - node1->data.sequence.items.start) !=
      ...
      return 0;
    for (k = 0; k < (node1->data.sequence.items.top -
...
int main(int argc, char *argv[]) {
  ...
  while (number < argc) {
    ...
    if (argv[number][0] == '-') {
      if (number < argc - 1) {
        memmove(argv + number, argv + number + 1,
                (argc - number - 1) * sizeof(char *));
      }
      argc--;
    } else {
      number++;
  ...
  for (number = 1; number < argc; number++) {
    ...
    printf("[%d] Loading, dumping, and loading again '%s': ", number,
           argv[number]);
    fflush(stdout);
    ...
    if (!error) {
      ...
      while (!done) {
        ...
        if (!done) {
          assert(compare_documents(documents + count, &document) ||
                 print_output(argv[number], buffer, written, count));
          count++;
        }
        yaml_document_delete(&document);
