...
#define DICTIONARY_LOGSIZE                                                     \
  28 /* Dictionary Size as a power of 2 (ex : 2^16 = 64K) */
...
#define ADD_HASH(p)                                                            \
  {                                                                            \
    NEXT_TRY(p) = HashTable[HASH_VALUE(p)];                                    \
    LEVEL_UP(p) = 0;                                                           \
    HashTable[HASH_VALUE(p)] = p;                                              \
  }
 ...
 *  Object Allocation
 ************************************************************/
MMC_ctx *MMC_create(void) { return (MMC_ctx *)ALLOCATOR(sizeof(MMC_ctx)); }
...
size_t MMC_init(MMC_ctx *MMC, const void *beginBuffer) {
  ...
  /* Init RLE detector */
  {
    ...
    for (c = 0; c < NBCHARACTERS; c++) {
      segmentInfo_t *const newSegment =
          REALLOCATOR(MMC->segments[c].segments,
                      NB_INITIAL_SEGMENTS * sizeof(segmentInfo_t));
      if (newSegment == NULL)
        return 1; /* realloc failed : existing segment is preserved */
      ...
      MMC->segments[c].segments[0].position =
          (const BYTE *)beginBuffer - (MAX_DISTANCE + 1);
    }
  }
  return 0;
}
...
void MMC_free(MMC_ctx *ctx) {
  int c;
  if (ctx == NULL)
    return;                          /* compatible free on NULL */
...
size_t MMC_insertAndFindBestMatch(MMC_ctx *MMC, const void *inputPointer,
                                  size_t maxLength, const void **matchpos) {
  ...
  if ((U16)sequence == (U16)(sequence >> 16) &&
    ...
    while ((*endSegment == c) && (endSegment < iend))
      endSegment++;
    nbChars = endSegment - ip;

    while (Segments[c].segments[index].size < nbChars)
      index--;

    if ((Segments[c].segments[index].position - nbChars) <=
        (ip - MAX_DISTANCE)) // no large enough previous serie within range
    {
    ...
    if (*(ip - 1) == c)
      ...
      *matchpos = ref; // "basis" to be improved upon
    if (nbChars == MINMATCH) {
      MMC_insert_once(MMC, ip, iend - ip);
      gateway = &LEVEL_UP(ip);
    }
  ...
  while ((ref) && ((ip - ref) < MAX_DISTANCE)) {
    ...
    mlt = MINMATCH;
    while ((mlt < (U32)maxLength) && (*(ip + mlt)) == *(ref + mlt))
      mlt++;
    ...
    if (mlt <= maxLevel) {
      NEXT_TRY(LEVEL(mlt)) = ref;
      LEVEL(mlt) = ref; // Completing chain at Level mlt
    }
    ...
    // New level creation
      ...
      // Special case : no gateway, but mlt>maxLevel
        ...
        gateway = &(LEVEL_UP(ref));
        NEXT_TRY(LEVEL(maxLevel)) = ref;
        LEVEL(maxLevel) = ref; // Completing chain at Level maxLevel
      }
    }

    {
      const BYTE *currentP = ref;
      NEXT_TRY(LEVEL(MINMATCH - 1)) =
          NEXT_TRY(ref); // Extraction from base level
      if (LEVEL_UP(ref)) {
        ref = LEVEL_UP(ref);
      ...
      ref = NEXT_TRY(ref);
      NEXT_TRY(currentP) =
          0; // initialisation, due to promotion; note that LEVEL_UP(ref)==0;
    }
  }
...
_FindBetterMatch:
  while ((ref) && ((ip - ref) < MAX_DISTANCE)) {
    ...
    if (mlt == currentLevel) {
      ...
      if (trackStep[c] == stepNb) {
        ...
        *trackPtr[c] = ref;                            // linking
        ...
        if (LEVEL_UP(ref)) {
          ...
          LEVEL_UP(ref) = 0;
          trackStep[c] = 0; // Shutdown chain (avoid overwriting when multiple
                            // unfinished chains)
        } else {
          NEXT_TRY(ref) =
              LEVEL_DOWN; // Promotion, but link back to previous level for now
      ...
      if (LEVEL_UP(ref) ==
          0) // don't interfere if a serie has already started...
             // Note : to "catch up" the serie, it would be necessary to scan
             // it, up to its last element this effort would be useless if the
             // chain is complete Alternatively : we could keep that gateway in
             // memory, and scan the chain on finding that it is not complete.
    ...
    _continue_same_level:
      ...
      if (ref == LEVEL_DOWN) {
        ...
        while (next > localCurrentP) {
          LEVEL(currentLevel - 1) = next;
          next = NEXT_TRY(next);
        }
        ref = next;
        currentLevel--;
    ...
    _check_mmc_levelup : {
      ...
      if (LEVEL_UP(ref)) {
        ...
        stepNb++;
        NEXT_TRY(LEVEL(currentLevel)) =
            ref; // We don't know yet ref's level, but just in case it would be
                 // only ==currentLevel...
      } else {
        ref = NEXT_TRY(ref);
        NEXT_TRY(currentP) =
            0; // promotion to level mlt; note that LEVEL_UP(ref)=0;
        if (ref == LEVEL_DOWN) {
          const BYTE *next = NEXT_TRY(LEVEL(currentLevel - 1));
          NEXT_TRY(LEVEL(currentLevel)) =
              0; // Erase the LEVEL_DOWN (which has been transfered)
          while (next > currentP) {
            LEVEL(currentLevel - 1) = next;
            next = NEXT_TRY(next);
          }
    ...
    // Special case : mlt>maxLevel==currentLevel, Level_up available, but no
    // gateway
    ...
    // Special case : mlt>maxLevel, but no gateway; Note that we don't know
    // about level_up yet
    gateway = &(LEVEL_UP(ref));
    NEXT_TRY(LEVEL(maxLevel)) = ref;
    LEVEL(maxLevel) = ref; // Completing chain of maxLevel
    goto _check_mmc_levelup;
  }

  if (gateway)
    *gateway = ip - MAX_DISTANCE - 1; // early end trick
  stepNb++;
...
static size_t MMC_insert_once(MMC_ctx *MMC, const void *ptr, size_t max) {
      ...
      (*ip == *(ip + 1))) /* 4 identical bytes */
  {
    ...
    if (endSegment == iend)
      return (iend -
              ip); /* skip the whole forward segment; we'll start again later */
    nbForwardChars = endSegment - ip;
    while ((baseStreamP > beginBuffer) && (baseStreamP[-1] == c))
      baseStreamP--;
    nbPreviousChars = ip - baseStreamP;
    segmentSize = nbForwardChars + nbPreviousChars;
    if (segmentSize > MAX_DISTANCE - 1)
      segmentSize = MAX_DISTANCE - 1;

    while (Segments[c].segments[Segments[c].start].size <= segmentSize) {
      if (Segments[c].segments[Segments[c].start].position <=
          (ip - MAX_DISTANCE))
        break;
      for (; n <= Segments[c].segments[Segments[c].start].size; n++) {
        NEXT_TRY(endSegment - n) =
            Segments[c].segments[Segments[c].start].position - n;
    ...
    /* overflow protection : new segment smaller than previous, but too many
     * segments in memory */
    if (Segments[c].start > Segments[c].max - 2) {
      segmentInfo_t *newSegment = REALLOCATOR(
          Segments[c].segments, (Segments[c].max * 2) * sizeof(segmentInfo_t));
      U32 beginning = 0;

      if (newSegment == NULL)
        return 0; /* allocation failed : we stop search here. Ideally, it should
                     be an error, rather than a soft "end of search" */
      Segments[c].max *= 2;
      Segments[c].segments = newSegment;
      /* transfer still valid positions (within window size) towards beginning
       * of buffer */
      while (newSegment[beginning].position < (ip - MAX_DISTANCE))
        beginning++;
      {
        U32 u;
        for (u = beginning; u <= Segments[c].start; u++) {
